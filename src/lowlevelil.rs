...
use crate::il::{Register, Flag, SSARegister, SSAFlag};
use crate::instruction::InstructionTextToken;
use crate::traits::{FunctionTrait, BasicBlockTrait};
use crate::wrappers::{BinjaLowLevelILFunction, BinjaFunction, BinjaBasicBlock};
...
impl LowLevelILFunction {
    pub fn new(func: Arc<BinjaFunction>) -> Result<LowLevelILFunction> {
        ...
        Ok(LowLevelILFunction{ handle: Arc::new(BinjaLowLevelILFunction::new(handle)), func })
    }
...
impl FunctionTrait for LowLevelILFunction {
    ...
    fn text(&self, i: u64) -> Result<String> {
        ...
        unsafe { 
            ...
            BNGetLowLevelILInstructionText(self.handle(), **self.func, self.arch()?.handle(), i, 
                                           &mut list, &mut count);

            if list.is_null() {
                println!("[Possible bug] Cannot retrieve LowLevelILInstruction Tokens: {} index {}", self, i);
...
impl LowLevelILBasicBlock {
    pub fn new(handle: *mut BNBasicBlock, func: LowLevelILFunction) -> Result<LowLevelILBasicBlock> {
        let handle = unsafe_try!(BNNewBasicBlockReference(handle))?;
        Ok(LowLevelILBasicBlock{ handle: Arc::new(BinjaBasicBlock::new(handle)), func })
    }
}
...
impl LowLevelILInstruction {
    ...
    pub fn from_func_index(func: LowLevelILFunction, instr_index: u64) -> LowLevelILInstruction { 
    ...
    pub fn from_expr(func: LowLevelILFunction, expr_index: u64, instr_index: Option<u64>) 
    ...
    pub fn ssa_form(&self) -> Result<LowLevelILInstruction> {
        ...
        unsafe {
            ...
            Ok(LowLevelILInstruction::from_expr(func_ssa, expr_index, self.instr_index))
        }
    }

    /// Get the MLIL instruction for this LLIL instruction
    pub fn medium_level_il(&self) -> Result<MediumLevelILInstruction> {
        let mlil_expr_index = unsafe {
            BNGetMediumLevelILExprIndex(self.function.handle(), self.expr_index) 
        };

        let mlil = self.function.function().mlil()?;

        Ok(MediumLevelILInstruction::from_expr(mlil, mlil_expr_index, None))
    }
...
pub enum LowLevelILOperation {
    Nop { },
    SetReg { dest: Register, src: LowLevelILInstruction, },
    SetRegSplit { hi: Register, lo: Register, src: LowLevelILInstruction, },
    SetFlag { dest: Flag, src: LowLevelILInstruction, },
    ...
    Load { src: LowLevelILInstruction, },
    Store { dest: LowLevelILInstruction, src: LowLevelILInstruction, },
    Push { src: LowLevelILInstruction, },
    Pop { },
    Reg { src: Register, },
    RegSplit { hi: Register, lo: Register, },
    // RegStackRel { stack: reg_stack, src: LowLevelILInstruction, },
    // RegStackPop { stack: reg_stack, },
    RegStackFreeReg { dest: Register, },
    ...
    Adc { left: LowLevelILInstruction, right: LowLevelILInstruction, carry: LowLevelILInstruction, },
    Sub { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Sbb { left: LowLevelILInstruction, right: LowLevelILInstruction, carry: LowLevelILInstruction, },
    And { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Or { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Xor { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Lsl { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Lsr { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Asr { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Rol { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Rlc { left: LowLevelILInstruction, right: LowLevelILInstruction, carry: LowLevelILInstruction, },
    Ror { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Rrc { left: LowLevelILInstruction, right: LowLevelILInstruction, carry: LowLevelILInstruction, },
    Mul { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    MuluDp { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    MulsDp { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Divu { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    DivuDp { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Divs { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    DivsDp { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Modu { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    ModuDp { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Mods { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    ModsDp { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Neg { src: LowLevelILInstruction, },
    Not { src: LowLevelILInstruction, },
    SignExtend { src: LowLevelILInstruction, },
    ZeroExtend { src: LowLevelILInstruction, },
    LowPart { src: LowLevelILInstruction, },
    Jump { dest: LowLevelILInstruction, },
    JumpTo { dest: LowLevelILInstruction, targets: HashMap<u64, u64>, },
    Call { dest: LowLevelILInstruction, },
    CallStackAdjust { dest: LowLevelILInstruction, stack_adjustment: u64, 
        reg_stack_adjustments: HashMap<u64, u32>, },
    Tailcall { dest: LowLevelILInstruction, },
    Ret { dest: LowLevelILInstruction, },
    Noret { },
    If { condition: LowLevelILInstruction, true_: u64, false_: u64, },
    Goto { dest: u64, },
    // FlagCond { condition: cond, semantic_class: sem_class, },
    // FlagGroup { semantic_group: sem_group, },
    Equals { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    NotEquals { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    SignedLessThan { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    UnsignedLessThan { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    SignedLessThanEquals { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    UnsignedLessThanEquals { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    SignedGreaterThanEquals { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    UnsignedGreaterThanEquals { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    SsignedGreaterThan { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    UnsignedGreaterThan { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    TestBit { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    BoolToInt { src: LowLevelILInstruction, },
    AddOverflow { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Syscall { },
    Bp { },
    Trap { vector: u64, },
    ...
    Undef { },
    Unimpl { },
    UnimplMem { src: LowLevelILInstruction, },
    Fadd { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Fsub { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Fmul { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Fdiv { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    Fsqrt { src: LowLevelILInstruction, },
    Fneg { src: LowLevelILInstruction, },
    Fabs { src: LowLevelILInstruction, },
    FloatToInt { src: LowLevelILInstruction, },
    IntToFloat { src: LowLevelILInstruction, },
    FloatConv { src: LowLevelILInstruction, },
    RoundToInt { src: LowLevelILInstruction, },
    Floor { src: LowLevelILInstruction, },
    Ceil { src: LowLevelILInstruction, },
    Ftrunc { src: LowLevelILInstruction, },
    FcmpE { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    FcmpNe { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    FcmpLt { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    FcmpLe { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    FcmpGe { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    FcmpGt { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    FcmpO { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    FcmpUo { left: LowLevelILInstruction, right: LowLevelILInstruction, },
    SetRegSsa { dest: SSARegister, src: LowLevelILInstruction, },
    SetRegSsaPartial { full_reg: SSARegister, dest: Register, src: LowLevelILInstruction, },
    SetRegSplitSsa { hi: LowLevelILInstruction, lo: LowLevelILInstruction, src: LowLevelILInstruction, },
    SetRegStackRelSsa { stack: LowLevelILInstruction, dest: LowLevelILInstruction, top: LowLevelILInstruction, src: LowLevelILInstruction, },
    SetRegStackAbsSsa { stack: LowLevelILInstruction, dest: Register, src: LowLevelILInstruction, },
    RegSplitDestSsa { dest: SSARegister, },
    // RegStackDestSsa { src: reg_stack_ssa_dest_and_src, },
    RegSsa { src: SSARegister, },
    RegSsaPartial { full_reg: SSARegister, src: Register, },
    RegSplitSsa { hi: SSARegister, lo: SSARegister, },
    ...
    RegStackFreeRelSsa { stack: LowLevelILInstruction, dest: LowLevelILInstruction, top: LowLevelILInstruction, },
    RegStackFreeAbsSsa { stack: LowLevelILInstruction, dest: Register, },
    SetFlagSsa { dest: SSAFlag, src: LowLevelILInstruction, },
    FlagSsa { src: SSAFlag, },
    FlagBitSsa { src: SSAFlag, bit: u64, },
    CallSsa { output: LowLevelILInstruction, dest: LowLevelILInstruction, stack: LowLevelILInstruction, param: LowLevelILInstruction, },
    SyscallSsa { output: LowLevelILInstruction, stack: LowLevelILInstruction, param: LowLevelILInstruction, },
    TailcallSsa { output: LowLevelILInstruction, dest: LowLevelILInstruction, stack: LowLevelILInstruction, param: LowLevelILInstruction, },
    CallParam { src: Vec<LowLevelILInstruction>, },
    CallStackSsa { src: SSARegister, src_memory: u64, },
    CallOutputSsa { dest_memory: u64, dest: Vec<SSARegister>, },
    LoadSsa { src: LowLevelILInstruction, src_memory: u64, },
    StoreSsa { dest: LowLevelILInstruction, dest_memory: u64, src_memory: u64, src: LowLevelILInstruction, },
    // IntrinsicSsa { output: reg_or_flag_ssa_list, intrinsic: intrinsic, param: LowLevelILInstruction, },
    RegPhi { dest: SSARegister, src: Vec<SSARegister>, },
    // RegStackPhi { dest: reg_stack_ssa, src: reg_stack_ssa_list, },
    FlagPhi { dest: SSAFlag, src: Vec<SSAFlag>, },
    MemPhi { dest_memory: u64, src_memory: Vec<u64>, },
}
impl LowLevelILOperation {
    pub fn from_instr(instr: BNLowLevelILInstruction, func: &LowLevelILFunction, expr_index: u64)
            -> LowLevelILOperation {
        ...
        macro_rules! expr {
            () => {{
                let res = LowLevelILInstruction::from_expr(func.clone(), instr.operands[operand_index], None);
                operand_index += 1;
                res
            }}
        }
        ...
        macro_rules! reg_ssa_list {
            () => {{
                ...
                unsafe { 
                    let operands = BNLowLevelILGetOperandList(func.handle(), expr_index, 
            ...
            () => {{
                ...
                unsafe { 
                    ...
                    for op in operands_slice {
                        let i = LowLevelILInstruction::from_expr(func.clone(), *op, None);
                        instrs.push(i);
                    }
        ...
        match instr.operation {
            BNLowLevelILOperation_LLIL_NOP => {
            ...
            BNLowLevelILOperation_LLIL_SET_REG_SPLIT => {
                ...
                let lo = reg!();
                let src = expr!();
                LowLevelILOperation::SetRegSplit {
                    hi, lo, src
                }
            }
            ...
            BNLowLevelILOperation_LLIL_STORE => {
                let dest = expr!();
                let src = expr!();
                LowLevelILOperation::Store {
                    dest, src
                }
            }
            BNLowLevelILOperation_LLIL_PUSH => {
                let src = expr!();
                LowLevelILOperation::Push {
                    src
                }
            }
            BNLowLevelILOperation_LLIL_POP => {
                LowLevelILOperation::Pop {
            ...
            BNLowLevelILOperation_LLIL_CONST => {
                let constant = int!();
                LowLevelILOperation::Const {
                    constant
                }
            }
            BNLowLevelILOperation_LLIL_CONST_PTR => {
                let constant = int!();
                LowLevelILOperation::ConstPtr {
                    constant
                }
            }
            ...
            BNLowLevelILOperation_LLIL_CALL_STACK_ADJUST => {
                ...
                LowLevelILOperation::CallStackAdjust {
                    dest, stack_adjustment, reg_stack_adjustments
                }
            }
            BNLowLevelILOperation_LLIL_TAILCALL => {
                let dest = expr!();
                LowLevelILOperation::Tailcall {
                    dest
                }
            }
            BNLowLevelILOperation_LLIL_RET => {
                let dest = expr!();
                LowLevelILOperation::Ret {
                    dest
                }
            }
            BNLowLevelILOperation_LLIL_NORET => {
                LowLevelILOperation::Noret {
            ...
            BNLowLevelILOperation_LLIL_CMP_SLT => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::SignedLessThan {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_CMP_ULT => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::UnsignedLessThan {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_CMP_SLE => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::SignedLessThanEquals {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_CMP_ULE => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::UnsignedLessThanEquals {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_CMP_SGE => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::SignedGreaterThanEquals {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_CMP_UGE => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::UnsignedGreaterThanEquals {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_CMP_SGT => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::SsignedGreaterThan {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_CMP_UGT => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::UnsignedGreaterThan {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_TEST_BIT => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::TestBit {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_BOOL_TO_INT => {
                let src = expr!();
                LowLevelILOperation::BoolToInt {
                    src
                }
            }
            BNLowLevelILOperation_LLIL_ADD_OVERFLOW => {
                let left = expr!();
                let right = expr!();
                LowLevelILOperation::AddOverflow {
                    left, right
                }
            }
            BNLowLevelILOperation_LLIL_SYSCALL => {
                LowLevelILOperation::Syscall {
                    
                }
            }
            BNLowLevelILOperation_LLIL_BP => {
                LowLevelILOperation::Bp {
                    
                }
            }
            BNLowLevelILOperation_LLIL_TRAP => {
                let vector = int!();
                LowLevelILOperation::Trap {
                    vector
                }
            }
            ...
            BNLowLevelILOperation_LLIL_UNDEF => {
                LowLevelILOperation::Undef {
                    
                }
            }
            BNLowLevelILOperation_LLIL_UNIMPL => {
                LowLevelILOperation::Unimpl {
            ...
            BNLowLevelILOperation_LLIL_SET_REG_SPLIT_SSA => {
                ...
                let lo = expr!();
                let src = expr!();
                LowLevelILOperation::SetRegSplitSsa {
                    hi, lo, src
                }
            }
            ...
            BNLowLevelILOperation_LLIL_SET_REG_STACK_ABS_SSA => {
                ...
                let dest = reg!();
                let src = expr!();
                LowLevelILOperation::SetRegStackAbsSsa {
                    stack, dest, src
                }
            }
            BNLowLevelILOperation_LLIL_REG_SPLIT_DEST_SSA => {
                let dest = reg_ssa!();
                LowLevelILOperation::RegSplitDestSsa {
                    dest
                }
            }
            ...
            BNLowLevelILOperation_LLIL_REG_SSA_PARTIAL => {
                let full_reg = reg_ssa!();
                let src = reg!();
                LowLevelILOperation::RegSsaPartial {
                    full_reg, src
                }
            }
            BNLowLevelILOperation_LLIL_REG_SPLIT_SSA => {
                let hi = reg_ssa!();
                let lo = reg_ssa!();
                LowLevelILOperation::RegSplitSsa {
                    hi, lo
                }
            }
            ...
            BNLowLevelILOperation_LLIL_REG_STACK_FREE_REL_SSA => {
                ...
                let dest = expr!();
                let top = expr!();
                LowLevelILOperation::RegStackFreeRelSsa {
                    stack, dest, top
                }
            }
            BNLowLevelILOperation_LLIL_REG_STACK_FREE_ABS_SSA => {
                let stack = expr!();
                let dest = reg!();
                LowLevelILOperation::RegStackFreeAbsSsa {
                    stack, dest
                }
            }
            BNLowLevelILOperation_LLIL_SET_FLAG_SSA => {
                let dest = flag_ssa!();
                let src = expr!();
                LowLevelILOperation::SetFlagSsa {
                    dest, src
                }
            }
            BNLowLevelILOperation_LLIL_FLAG_SSA => {
                let src = flag_ssa!();
                LowLevelILOperation::FlagSsa {
                    src
                }
            }
            BNLowLevelILOperation_LLIL_FLAG_BIT_SSA => {
                let src = flag_ssa!();
                let bit = int!();
                LowLevelILOperation::FlagBitSsa {
                    src, bit
                }
            }
            ...
            BNLowLevelILOperation_LLIL_CALL_STACK_SSA => {
                let src = reg_ssa!();
                let src_memory = int!();
                LowLevelILOperation::CallStackSsa {
                    src, src_memory
                }
            }
            BNLowLevelILOperation_LLIL_CALL_OUTPUT_SSA => {
                ...
                LowLevelILOperation::CallOutputSsa {
                    dest_memory, dest
                }
            }
