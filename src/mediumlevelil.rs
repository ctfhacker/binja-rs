...
impl FunctionTrait for MediumLevelILFunction {
    ...
    fn blocks(&self) -> Vec<Self::Block> {
        ...
        unsafe {
            ...
            let blocks_slice = slice::from_raw_parts(blocks, count);
...
impl MediumLevelILBasicBlock {
    pub fn new(
        ...
        func: MediumLevelILFunction,
    ) -> Result<MediumLevelILBasicBlock> {
        let handle = unsafe_try!(BNNewBasicBlockReference(handle))?;
        Ok(MediumLevelILBasicBlock {
...
impl MediumLevelILInstruction {
    ...
    pub fn from_expr(
        ...
        if instr_index.is_none() {
            instr_index =
                Some(unsafe { BNGetMediumLevelILInstructionForExpr(func.handle(), expr_index) });
        }
    ...
    pub fn ssa_form(&self) -> Result<MediumLevelILInstruction> {
        ...
        let expr_index =
            unsafe { BNGetMediumLevelILSSAExprIndex(self.function.handle(), self.expr_index) };
    ...
    pub fn high_level_il(&self) -> Result<HighLevelILInstruction> {
        let hlil_expr_index =
            unsafe { BNGetHighLevelILExprIndex(self.function.handle(), self.expr_index) };
    ...
    pub fn basic_block(&self) -> Result<BasicBlock> {
        if self.instr_index.is_none() {
            return Err(anyhow!(
                "Cannot create MLIL basic block without instr_index"
            ));
        }

        let handle = unsafe_try!(BNGetMediumLevelILBasicBlockForInstruction(
            self.function.handle(),
            self.instr_index.unwrap()
        ))?;
...
pub enum MediumLevelILOperation {
    ...
    Adc {
        left: MediumLevelILInstruction,
        right: MediumLevelILInstruction,
        carry: MediumLevelILInstruction,
    },
    Sub {
        left: MediumLevelILInstruction,
        right: MediumLevelILInstruction,
    ...
    CallUntyped {
        output: MediumLevelILInstruction,
        dest: MediumLevelILInstruction,
        params: MediumLevelILInstruction,
        stack: MediumLevelILInstruction,
    },
    CallOutput {
        dest: Vec<Variable>,
    },
...
impl MediumLevelILOperation {
    pub fn from_instr(
        ...
        macro_rules! expr {
            () => {{
                let res = MediumLevelILInstruction::from_expr(
                    func.clone(),
        ...
        macro_rules! expr_list {
            () => {{
                ...
                unsafe {
                    ...
                    let operands = BNMediumLevelILGetOperandList(
                        func.handle(),
                        expr_index,
                        operand_index,
                        &mut count,
                    ...
                    for op in operands_slice {
                        let i =
                            MediumLevelILInstruction::from_expr(func.clone(), *op as usize, None);
                        instrs.push(i);
                    }
        ...
        macro_rules! var_ssa {
            () => {{
                let var = var!();
                let version = instr.operands[operand_index].try_into().unwrap();
                operand_index += 1;
                operand_index += 1;
        ...
        macro_rules! var_ssa_list {
            () => {{
                ...
                unsafe {
                    ...
                    for i in (0..count).step_by(2) {
                        let id = operands_slice[i as usize];
                        let version = operands_slice[i as usize + 1].try_into().unwrap();
                        let bnvar = BNFromVariableIdentifier(id);
                        let var = Variable::new(func.function(), bnvar);
        ...
        macro_rules! var_ssa_dest_and_src {
            () => {{
                ...
                operand_index += 2;
                let dest_ssa = SSAVariable::new(var.clone(), dest_version.try_into().unwrap());
                let src_ssa = SSAVariable::new(var, src_version.try_into().unwrap());
        ...
        macro_rules! constant_data {
            () => {{
                ...
                let value = instr.operands[operand_index + 1];
                operand_index += 2;
        ...
        let res = match instr.operation {
            BNMediumLevelILOperation_MLIL_NOP => MediumLevelILOperation::Nop {},
            ...
            BNMediumLevelILOperation_MLIL_SET_VAR_FIELD => {
                ...
                let offset = int!();
                let src = expr!();
                MediumLevelILOperation::SetVarField { dest, offset, src }
            }
            BNMediumLevelILOperation_MLIL_SET_VAR_SPLIT => {
                let high = var!();
                let low = var!();
                let src = expr!();
                MediumLevelILOperation::SetVarSplit { high, low, src }
            }
            BNMediumLevelILOperation_MLIL_LOAD => {
                let src = expr!();
                MediumLevelILOperation::Load { src }
            }
            BNMediumLevelILOperation_MLIL_LOAD_STRUCT => {
                let src = expr!();
                let offset = int!();
                MediumLevelILOperation::LoadStruct { src, offset }
            }
            BNMediumLevelILOperation_MLIL_STORE => {
                let dest = expr!();
                let src = expr!();
                MediumLevelILOperation::Store { dest, src }
            }
            BNMediumLevelILOperation_MLIL_STORE_STRUCT => {
                let dest = expr!();
                let offset = int!();
                let src = expr!();
                MediumLevelILOperation::StoreStruct { dest, offset, src }
            }
            ...
            BNMediumLevelILOperation_MLIL_VAR_FIELD => {
                let src = var!();
                let offset = int!();
                MediumLevelILOperation::VarField { src, offset }
            }
            BNMediumLevelILOperation_MLIL_VAR_SPLIT => {
                let high = var!();
                let low = var!();
                MediumLevelILOperation::VarSplit { high, low }
            }
            BNMediumLevelILOperation_MLIL_ADDRESS_OF => {
                let src = var!();
                MediumLevelILOperation::AddressOf { src }
            }
            BNMediumLevelILOperation_MLIL_ADDRESS_OF_FIELD => {
                let src = var!();
                let offset = int!();
                MediumLevelILOperation::AddressOfField { src, offset }
            }
            BNMediumLevelILOperation_MLIL_CONST => {
                let constant = int!();
                MediumLevelILOperation::Const { constant }
            }
            ...
            BNMediumLevelILOperation_MLIL_EXTERN_PTR => {
                let constant = int!();
                let offset = int!();
                MediumLevelILOperation::ExternPtr { constant, offset }
            }
            ...
            BNMediumLevelILOperation_MLIL_IMPORT => {
                let constant = int!();
                MediumLevelILOperation::Import { constant }
            }
            ...
            BNMediumLevelILOperation_MLIL_ADC => {
                ...
                let right = expr!();
                let carry = expr!();
                MediumLevelILOperation::Adc { left, right, carry }
            }
            BNMediumLevelILOperation_MLIL_SUB => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Sub { left, right }
            }
            BNMediumLevelILOperation_MLIL_SBB => {
                let left = expr!();
                let right = expr!();
                let carry = expr!();
                MediumLevelILOperation::Sbb { left, right, carry }
            }
            BNMediumLevelILOperation_MLIL_AND => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::And { left, right }
            }
            BNMediumLevelILOperation_MLIL_OR => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Or { left, right }
            }
            BNMediumLevelILOperation_MLIL_XOR => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Xor { left, right }
            }
            BNMediumLevelILOperation_MLIL_LSL => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Lsl { left, right }
            }
            BNMediumLevelILOperation_MLIL_LSR => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Lsr { left, right }
            }
            BNMediumLevelILOperation_MLIL_ASR => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Asr { left, right }
            }
            BNMediumLevelILOperation_MLIL_ROL => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Rol { left, right }
            }
            BNMediumLevelILOperation_MLIL_RLC => {
                let left = expr!();
                let right = expr!();
                let carry = expr!();
                MediumLevelILOperation::Rlc { left, right, carry }
            }
            BNMediumLevelILOperation_MLIL_ROR => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Ror { left, right }
            }
            BNMediumLevelILOperation_MLIL_RRC => {
                let left = expr!();
                let right = expr!();
                let carry = expr!();
                MediumLevelILOperation::Rrc { left, right, carry }
            }
            BNMediumLevelILOperation_MLIL_MUL => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Mul { left, right }
            }
            BNMediumLevelILOperation_MLIL_MULU_DP => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::MuluDp { left, right }
            }
            BNMediumLevelILOperation_MLIL_MULS_DP => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::MulsDp { left, right }
            }
            BNMediumLevelILOperation_MLIL_DIVU => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Divu { left, right }
            }
            BNMediumLevelILOperation_MLIL_DIVU_DP => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::DivuDp { left, right }
            }
            BNMediumLevelILOperation_MLIL_DIVS => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Divs { left, right }
            }
            BNMediumLevelILOperation_MLIL_DIVS_DP => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::DivsDp { left, right }
            }
            BNMediumLevelILOperation_MLIL_MODU => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Modu { left, right }
            }
            BNMediumLevelILOperation_MLIL_MODU_DP => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::ModuDp { left, right }
            }
            BNMediumLevelILOperation_MLIL_MODS => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Mods { left, right }
            }
            BNMediumLevelILOperation_MLIL_MODS_DP => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::ModsDp { left, right }
            }
            BNMediumLevelILOperation_MLIL_NEG => {
                let src = expr!();
                MediumLevelILOperation::Neg { src }
            }
            BNMediumLevelILOperation_MLIL_NOT => {
                let src = expr!();
                MediumLevelILOperation::Not { src }
            }
            BNMediumLevelILOperation_MLIL_SX => {
                let src = expr!();
                MediumLevelILOperation::SignExtend { src }
            }
            BNMediumLevelILOperation_MLIL_ZX => {
                let src = expr!();
                MediumLevelILOperation::ZeroExtend { src }
            }
            BNMediumLevelILOperation_MLIL_LOW_PART => {
                let src = expr!();
                MediumLevelILOperation::LowPart { src }
            }
            BNMediumLevelILOperation_MLIL_JUMP => {
                let dest = expr!();
                MediumLevelILOperation::Jump { dest }
            }
            BNMediumLevelILOperation_MLIL_JUMP_TO => {
                let dest = expr!();
                let targets = target_map!();
                MediumLevelILOperation::JumpTo { dest, targets }
            }
            BNMediumLevelILOperation_MLIL_RET_HINT => {
                let dest = expr!();
                MediumLevelILOperation::RetHint { dest }
            }
            ...
            BNMediumLevelILOperation_MLIL_CALL_OUTPUT => {
                let dest = var_list!();
                MediumLevelILOperation::CallOutput { dest }
            }
            BNMediumLevelILOperation_MLIL_CALL_PARAM => {
                let src = var_list!();
                MediumLevelILOperation::CallParam { src }
            }
            BNMediumLevelILOperation_MLIL_RET => {
                let src = expr_list!();
                MediumLevelILOperation::Ret { src }
            }
            BNMediumLevelILOperation_MLIL_NORET => MediumLevelILOperation::Noret {},
            ...
            BNMediumLevelILOperation_MLIL_CMP_E => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Equals { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_NE => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::NotEquals { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_SLT => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::SignedLessThan { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_ULT => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::UnsignedLessThan { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_SLE => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::SignedLessThanEquals { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_ULE => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::UnsignedLessThanEquals { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_SGE => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::SignedGreaterThanEquals { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_UGE => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::UnsignedGreaterThanEquals { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_SGT => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::SsignedGreaterThan { left, right }
            }
            BNMediumLevelILOperation_MLIL_CMP_UGT => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::UnsignedGreaterThan { left, right }
            }
            BNMediumLevelILOperation_MLIL_TEST_BIT => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::TestBit { left, right }
            }
            BNMediumLevelILOperation_MLIL_BOOL_TO_INT => {
                let src = expr!();
                MediumLevelILOperation::BoolToInt { src }
            }
            BNMediumLevelILOperation_MLIL_ADD_OVERFLOW => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::AddOverflow { left, right }
            }
            BNMediumLevelILOperation_MLIL_SYSCALL => {
                let output = var_list!();
                let params = expr_list!();
                MediumLevelILOperation::Syscall { output, params }
            }
            ...
            BNMediumLevelILOperation_MLIL_FREE_VAR_SLOT => {
                let dest = var!();
                MediumLevelILOperation::FreeVarSlot { dest }
            }
            BNMediumLevelILOperation_MLIL_BP => MediumLevelILOperation::Bp {},
            BNMediumLevelILOperation_MLIL_TRAP => {
                let vector = int!();
                MediumLevelILOperation::Trap { vector }
            }
            BNMediumLevelILOperation_MLIL_UNDEF => MediumLevelILOperation::Undef {},
            BNMediumLevelILOperation_MLIL_UNIMPL => MediumLevelILOperation::Unimpl {},
            BNMediumLevelILOperation_MLIL_UNIMPL_MEM => {
                let src = expr!();
                MediumLevelILOperation::UnimplMem { src }
            }
            BNMediumLevelILOperation_MLIL_FADD => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Fadd { left, right }
            }
            BNMediumLevelILOperation_MLIL_FSUB => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Fsub { left, right }
            }
            BNMediumLevelILOperation_MLIL_FMUL => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Fmul { left, right }
            }
            BNMediumLevelILOperation_MLIL_FDIV => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::Fdiv { left, right }
            }
            BNMediumLevelILOperation_MLIL_FSQRT => {
                let src = expr!();
                MediumLevelILOperation::Fsqrt { src }
            }
            BNMediumLevelILOperation_MLIL_FNEG => {
                let src = expr!();
                MediumLevelILOperation::Fneg { src }
            }
            BNMediumLevelILOperation_MLIL_FABS => {
                let src = expr!();
                MediumLevelILOperation::Fabs { src }
            }
            BNMediumLevelILOperation_MLIL_FLOAT_TO_INT => {
                let src = expr!();
                MediumLevelILOperation::FloatToInt { src }
            }
            BNMediumLevelILOperation_MLIL_INT_TO_FLOAT => {
                let src = expr!();
                MediumLevelILOperation::IntToFloat { src }
            }
            BNMediumLevelILOperation_MLIL_FLOAT_CONV => {
                let src = expr!();
                MediumLevelILOperation::FloatConv { src }
            }
            BNMediumLevelILOperation_MLIL_ROUND_TO_INT => {
                let src = expr!();
                MediumLevelILOperation::RoundToInt { src }
            }
            BNMediumLevelILOperation_MLIL_FLOOR => {
                let src = expr!();
                MediumLevelILOperation::Floor { src }
            }
            BNMediumLevelILOperation_MLIL_CEIL => {
                let src = expr!();
                MediumLevelILOperation::Ceil { src }
            }
            BNMediumLevelILOperation_MLIL_FTRUNC => {
                let src = expr!();
                MediumLevelILOperation::Ftrunc { src }
            }
            BNMediumLevelILOperation_MLIL_FCMP_E => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::FcmpE { left, right }
            }
            BNMediumLevelILOperation_MLIL_FCMP_NE => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::FcmpNe { left, right }
            }
            BNMediumLevelILOperation_MLIL_FCMP_LT => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::FcmpLt { left, right }
            }
            BNMediumLevelILOperation_MLIL_FCMP_LE => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::FcmpLe { left, right }
            }
            BNMediumLevelILOperation_MLIL_FCMP_GE => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::FcmpGe { left, right }
            }
            BNMediumLevelILOperation_MLIL_FCMP_GT => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::FcmpGt { left, right }
            }
            BNMediumLevelILOperation_MLIL_FCMP_O => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::FcmpO { left, right }
            }
            BNMediumLevelILOperation_MLIL_FCMP_UO => {
                let left = expr!();
                let right = expr!();
                MediumLevelILOperation::FcmpUo { left, right }
            }
            BNMediumLevelILOperation_MLIL_SET_VAR_SSA => {
                let dest = var_ssa!();
                let src = expr!();
                MediumLevelILOperation::SetVarSsa { dest, src }
            }
            BNMediumLevelILOperation_MLIL_SET_VAR_SSA_FIELD => {
                let prev = var_ssa_dest_and_src!();
                let offset = int!();
                let src = expr!();
                MediumLevelILOperation::SetVarSsaField { prev, offset, src }
            }
            BNMediumLevelILOperation_MLIL_SET_VAR_SPLIT_SSA => {
                let high = var_ssa!();
                let low = var_ssa!();
                let src = expr!();
                MediumLevelILOperation::SetVarSplitSsa { high, low, src }
            }
            BNMediumLevelILOperation_MLIL_SET_VAR_ALIASED => {
                let prev = var_ssa_dest_and_src!();
                let src = expr!();
                MediumLevelILOperation::SetVarAliased { prev, src }
            }
            BNMediumLevelILOperation_MLIL_SET_VAR_ALIASED_FIELD => {
                let prev = var_ssa_dest_and_src!();
                let offset = int!();
                let src = expr!();
                MediumLevelILOperation::SetVarAliasedField { prev, offset, src }
            }
            BNMediumLevelILOperation_MLIL_VAR_SSA => {
                let src = var_ssa!();
                MediumLevelILOperation::VarSsa { src }
            }
            BNMediumLevelILOperation_MLIL_VAR_SSA_FIELD => {
                let src = var_ssa!();
                let offset = int!();
                MediumLevelILOperation::VarSsaField { src, offset }
            }
            BNMediumLevelILOperation_MLIL_VAR_ALIASED => {
                let src = var_ssa!();
                MediumLevelILOperation::VarAliased { src }
            }
            BNMediumLevelILOperation_MLIL_VAR_ALIASED_FIELD => {
                let src = var_ssa!();
                let offset = int!();
                MediumLevelILOperation::VarAliasedField { src, offset }
            }
            BNMediumLevelILOperation_MLIL_VAR_SPLIT_SSA => {
                let high = var_ssa!();
                let low = var_ssa!();
                MediumLevelILOperation::VarSplitSsa { high, low }
            }
            ...
            BNMediumLevelILOperation_MLIL_CALL_PARAM_SSA => {
                ...
                MediumLevelILOperation::CallParamSsa { src_memory, src }
            }
            BNMediumLevelILOperation_MLIL_CALL_OUTPUT_SSA => {
                let dest_memory = int!();
                let dest = var_ssa_list!();
                MediumLevelILOperation::CallOutputSsa { dest_memory, dest }
            }
            BNMediumLevelILOperation_MLIL_LOAD_SSA => {
                let src = expr!();
                let src_memory = int!();
                MediumLevelILOperation::LoadSsa { src, src_memory }
            }
            ...
            BNMediumLevelILOperation_MLIL_FREE_VAR_SLOT_SSA => {
                let prev = var_ssa_dest_and_src!();
                MediumLevelILOperation::FreeVarSlotSsa { prev }
            }
            BNMediumLevelILOperation_MLIL_VAR_PHI => {
                let dest = var_ssa!();
                let src = var_ssa_list!();
                MediumLevelILOperation::VarPhi { dest, src }
            }
            ...
            BNMediumLevelILOperation_MLIL_CONST_DATA => {
                let constant_data = constant_data!();
                MediumLevelILOperation::ConstData { constant_data }
            }
            x => panic!("Unknown MLIL Operation: {x}"),
        };
    ...
    pub fn ssa_vars(&self) -> Option<Vec<SSAVariable>> {
        let res = match self {
            ...
            MediumLevelILOperation::Add {
                /* MediumLevelILInstruction */ left,
                /* MediumLevelILInstruction */ right,
            } => {
                let mut res = Vec::new();
            ...
            MediumLevelILOperation::Adc {
                /* MediumLevelILInstruction */ left,
                /* MediumLevelILInstruction */ right,
                /* MediumLevelILInstruction */ carry,
            } => {
                let mut res = Vec::new();
                if let Some(vars) = left.ssa_vars() {
                    for var in vars {
            ...
            MediumLevelILOperation::Call {
                /* Vec<Variable> */ output,
                /* MediumLevelILInstruction */ dest,
                /* Vec<MediumLevelILInstruction> */ params,
            } => {
                let mut res = Vec::new();
                if let Some(vars) = dest.ssa_vars() {
                    for var in vars {
            ...
            MediumLevelILOperation::CallUntyped {
                /* MediumLevelILInstruction */ output,
                /* MediumLevelILInstruction */ dest,
                /* MediumLevelILInstruction */ params,
                /* MediumLevelILInstruction */ stack,
            } => {
                let mut res = Vec::new();
            ...
            MediumLevelILOperation::Ret {
                /* Vec<MediumLevelILInstruction> */ src,
            } => {
                let mut res = Vec::new();
                for instr in src {
                    if let Some(vars) = instr.ssa_vars() {
            ...
            MediumLevelILOperation::ConstData {
    ...
    pub fn name(&self) -> &'static str {
        match self {
            ...
            MediumLevelILOperation::ConstData { .. } => "ConstData",
        }
    }
}
